...
class DecisionController:
    ...
    def load_inventory(self, xlsx_path: str) -> int:
        # The "inventory" is now the master sheet (previously called the alternates file).
        # We dedupe by internal itemnum and attach manufacturer alternates (substitutes)
        # to the base InventoryPart objects.
        ...
        # Treat master inventory as the alternates DB too (MPN -> [base itemnum...])
...
#
    ...
    def _pair_to_node(self, npr_part: Any, match: Any) -> DecisionNode:
        ...
        for cand in candidates:
            ...
            # ---- Phase A: show collapsed MFGPN alias count + carry meta into Alternate ----
            ...
            # ------------------------------------------------------------
            # IMPORTANT:
            # Candidates coming out of MatchingEngine may NOT be the same
            # object instances as the master-inventory records (which contain
            # substitutes / alternates).
            #
            # Rebind candidate -> master inventory by itemnum so EVERY card
            # can show MFGPNs and specs consistently.
            # ------------------------------------------------------------
            ...
            # use the resolved object for Alternate creation
            alt_obj = self._inventory_part_to_alternate(cand_res, confidence=conf, relationship=rel)

            # ------------------------------------------------------------
            # Compute MFGPN list/count from MASTER inventory:
            #   base vendoritem + substitute mpns
            # This is what you actually mean by "company PN owns alternates".
            # ------------------------------------------------------------
            base_vendoritem = (getattr(inv_for_ui, "vendoritem", "") or "").strip()
            subs = getattr(inv_for_ui, "substitutes", None) or []

            mpn_list = []
            if base_vendoritem:
                mpn_list.append(base_vendoritem)
            ...
            # dedupe keep order (case-insensitive)
            seen = set()
            mpn_list_dedup = []
            for m in mpn_list:
                k = m.lower()
                if k in seen:
                    continue
                seen.add(k)
                mpn_list_dedup.append(m)

            # keep your meta stamping logic as-is (but now it will apply to the canonical object)
