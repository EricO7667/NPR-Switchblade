...
class DecisionWorkspaceUI:
    ...
    def _build_toolbar(self):
            ...
            add_btn("Open Workspace", self._open_workspace)
            add_btn("Save Workspace", self._save_workspace)
            ...
            add_btn("Re-run Matching (Preserve Decisions)", self._rematch_workspace)
    ...
    def _open_workspace(self):
        try:
            ...
            for w in wss:
                ...
                bom_id = w.get("bom_id", "") or "-"
                line = f"{name}  |  updated: {updated}  |  bom: {bom_id}  |  {ws_id}"
                display.append(line)
                id_by_display[line] = ws_id

            var = tk.StringVar(value=display[0])
            combo = ttk.Combobox(win, textvariable=var, values=display, state="readonly")
            combo.pack(fill="x", padx=16, pady=(0, 12))

            info = tk.Label(
                win,
                text="Opening a workspace replaces the current in-memory nodes with the saved state.",
                font=("Segoe UI", 10),
                bg=self.COLORS["bg_main"],
    ...
    def _save_workspace(self):
        try:
            if not getattr(self.controller, "workspace_id", None):
                messagebox.showinfo(
                    "No Workspace",
                    "There is no active workspace to save yet."
                )
                return
        ...
        try:
            ...
            def run_task():
                try:
                    ...
                    self._safe_after(lambda err=e: messagebox.showerror("Rematch Failed", str(err)))
                finally:
                    self._safe_after(self._hide_loading_overlay)
            ...
            traceback.print_exc()
            messagebox.showerror("Rematch Failed", str(e))
    ...
    def _export_npr(self):
        ...
        try:
            ...
            messagebox.showerror("Export Failed. Please save your npr data and work manually before closing.", str(e))
    ...
    def _on_close(self):
        ...
        # Force-save current workspace on close (so dev/testing isn't slowed down)
        try:
            if self.controller:
                self.controller.flush_workspace_to_db()
        except Exception:
