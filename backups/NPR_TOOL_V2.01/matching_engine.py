...
class MatchingEngine:
    ...
    def _init_embeddings_cache(self, *, force: bool = False):
        ...
        if missing:
            ...
            try:
                ...
                np.savez_compressed(tmp_vec, keys=keys, vecs=vecs)
    ...
    def _engineering_match(self, npr: NPRPart) -> Optional[MatchResult]:
        ...
        # Seed each candidate with its semantic similarity score so DecisionController can rank/display it.
        for inv, score in zip(top_inv, top_scores):
            try:
                inv._pc_seed = float(score)  # 0..1 cosine similarity (since you normalized embeddings)
            except Exception:
                pass
