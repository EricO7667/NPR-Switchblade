...
#this can likley be applied in mathcing engine rather than here in the decsion controller,
def _norm_pkg(pkg: str) -> str:
    ...
    if p in ControllerConfig._IMPERIAL_PKGS:
        return p
    if p.isdigit() and len(p) == 4:
        return ControllerConfig._METRIC_TO_IMPERIAL.get(p, p)
    return p
    ...
    def run_matching(self) -> int:
        ...
        for n_i, npr in enumerate(self.npr_list):
            ...
            try:
                ...
                out = self._filter_rank_cap_candidates(npr_part, out, max_n=ControllerConfig.MAX_INTERNAL_CANDIDATES)
