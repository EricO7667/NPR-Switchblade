...
# ----------------------------
# Candidate limiting + strict filters
# ----------------------------
...
def _norm_pkg(pkg: str) -> str:
    """
    Normalize package codes so 2012->0805, 1608->0603, etc.
    Keeps unknown tokens as-is.
    """
    p = (pkg or "").strip().upper()
    p = p.replace(" ", "").replace("-", "")
    # common forms: "0805", "2012", "0603"
    if p in _IMPERIAL_PKGS:
        return p
    if p.isdigit() and len(p) == 4:
        return _METRIC_TO_IMPERIAL.get(p, p)
    return p

def _norm_res_value(v: str) -> str:
    """
    Normalize resistor value string to an ohms-ish canonical token.
    Examples: "10k" -> "10000", "10K0" -> "10000", "4R7" -> "4.7"
    Returns "" if unknown.
    """
    s = (v or "").strip().upper().replace(",", "")
    if not s:
        return ""
    # 4R7, 10K0, 1M0 patterns
    ...
    # 10K0 -> 10.0K
    m = re.match(r"^(\d+)([KMR])(\d+)$", s)
    if m:
        a, mult, b = m.groups()
        s = f"{a}.{b}{mult}"

    # 10K, 2.2M, 100, 10000
    m = re.match(r"^(\d+(\.\d+)?)([KMR])?$", s)
    if not m:
        return ""
    ...
    # stringify without trailing .0 when possible
    if abs(num - round(num)) < 1e-9:
        return str(int(round(num)))
    return str(num)

def _norm_cap_value(v: str) -> str:
    """
    Normalize capacitor value to farads in scientific-ish token string.
    Examples: "0.1uF" == "100nF" => same token (~1e-7 F)
    Returns "" if unknown.
    """
    s = (v or "").strip().upper().replace(",", "")
    if not s:
        return ""
    s = s.replace("UF", "U").replace("NF", "N").replace("PF", "P").replace("F", "")
    m = re.match(r"^(\d+(\.\d+)?)(P|N|U|M)?$", s)
    if not m:
        return ""
    num = float(m.group(1))
    mult = m.group(3) or ""
    ...
    # quantize a bit so floating fuzz doesn't break equality
    return f"{num:.12g}"

def _ptype(parsed: dict) -> str:
    return str((parsed or {}).get("type") or "OTHER").upper()

def _get_parsed(obj) -> dict:
    return getattr(obj, "parsed", {}) or {}
...
# current running issue: winning choices for boms with multple alternates is skibidiing around an not showingcasing the correct thing. dispaly for the Winner is actuallly being poopy and disrecpecting me. by this i mean its displaying the winner, but NOT displaying the winners matches. an issue is that its not finding a matches for the displayed parts in the ui and IS showing the matches of the nondisplayed part. fo some reason the non displayed part is the winner???????? wth.
    def run_matching(self) -> int:
        ...
        # ---------------------------------------------------------
        # One engine per run + one semantic cache build
        # ---------------------------------------------------------
        ...
        engine = MatchingEngine(self.inventory, config=cfg, ui_root=tk_root)
        ...
        for n_i, npr in enumerate(self.npr_list):
            ...
            try:
                ...
                # Hard-filter + rerank + cap
                out = self._filter_rank_cap_candidates(npr_part, out, max_n=MAX_INTERNAL_CANDIDATES)
        ...
        # --- PATCH: sync winning_mpn and attempt flags ---
        try:
    ...
    def _filter_rank_cap_candidates(self, npr_part: Any, candidates: list, max_n: int = MAX_INTERNAL_CANDIDATES) -> list:
        """
    ...
    def _pair_to_node(self, npr_part: Any, match: Any) -> DecisionNode:
        ...
        ptype = str(parsed.get("type") or "OTHER") if isinstance(parsed, dict) else "OTHER"
        ...
        # now safe to set dynamic attributes
