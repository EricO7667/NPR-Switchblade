...
class DecisionWorkspaceUI:
    ...
    def _load_alternates_db(self):
        ...
        if not path:
            return
        try:
            attached, unique_mpns = self.controller.load_alternates_db(path)
            self.status_var.set(
                f"Loaded alternates DB: {attached} base parts enriched, {unique_mpns} unique MFGPN keys."
            )
            # If something is currently selected, refresh the detail pane so alternates show immediately
            if getattr(self, "selected_node", None):
                self._refresh_detail_for_selected()
        except Exception as e:
            messagebox.showerror("Load Alternates DB Failed", str(e))
    ...
    def _build_toolbar(self):
            ...
            add_btn("Load Alternates DB", self._load_alternates_db)   
...
#
    ...
    def _render_specs(self, specs: dict):
            ...
            if "AlternatesCount" in specs or "AlternatesList" in specs:
                add_row(self.specs_inner, "ALTERNATES", "", is_header=True)
                if "AlternatesCount" in specs:
                    add_row(self.specs_inner, "Count", specs["AlternatesCount"])
                if "AlternatesList" in specs:
                    add_row(self.specs_inner, "List", specs["AlternatesList"])
            
            # 5. Everything else (debug / extra fields)
            add_row(self.specs_inner, "OTHER", "", is_header=True)
            #known = {
            #    "ItemNumber","VendorItem","Description","MfgName","MfgId","PrimaryVendorNumber",
            #    "TotalQty","LastCost","AvgCost","ItemLeadTime","DefaultWhse","TariffCodeHTSUS",
            #    "AlternatesCount","AlternatesList"  
            #}
    ...
    def _specs_from_inventory(self, inv):
        ...
        # ---- Alternates (Substitutes) from Alternates DB ----
        subs = getattr(inv, "substitutes", None) or []
        if subs:
            specs["AlternatesCount"] = str(len(subs))

            # Render a readable list; cap to keep UI fast
            MAX_SHOW = 30
            lines = []
            for s in subs[:MAX_SHOW]:
                mpn = (getattr(s, "mfgpn", "") or "").strip()
                dsc = (getattr(s, "description", "") or "").strip()
                base = (getattr(s, "base_itemnum", "") or "").strip()
                # base should equal inv.itemnum; but include mpn + desc for usefulness
                if mpn and dsc:
                    lines.append(f"{mpn} — {dsc}")
                elif mpn:
                    lines.append(mpn)
                elif dsc:
                    lines.append(dsc)
            ...
            if len(subs) > MAX_SHOW:
                lines.append(f"... +{len(subs) - MAX_SHOW} more")
    ...
    def _create_card(self, node: DecisionNode, alt: Alternate) -> tk.Frame:
            ...
            # --- LEFT SIDE: MPN + AlternatesCount badge (if inventory) ---
            ...
            # If this card is backed by an InventoryPart, show its alternates count
            alt_count = 0
            if alt.raw is not None:
            ...
            if alt_count > 0:
                tk.Label(
                    left_box,
                    text=f"Alts: {alt_count}",
                    font=("Segoe UI", 8, "bold"),
                    bg="#F3F4F6",   # light gray pill
                    fg="#374151",
                    padx=8,
                    pady=2,
