...
class DecisionController:
    ...
    def load_inventory(self, xlsx_path: str) -> int:
        ...
        self._inv_by_itemnum = {inv.itemnum: inv for inv in (self.inventory or []) if getattr(inv, "itemnum", "")}
        return len(self.inventory)
    
    def load_alternates_db(self, xlsx_path: str) -> tuple[int, int]:
        """
        Loads the master manufacturer cross-reference export, attaches non-self substitutes onto inventory parts,
        and builds MFGPN->base lookup.

        Returns: (num_base_items_with_added_subs, num_unique_mfgpn_keys)
        """

        if not self.inventory:
            raise RuntimeError("Load inventory before loading alternates DB (needs base itemnum objects).")

        subs_by_base, mpn_to_base = DataLoader.load_alternates_db(xlsx_path)

        # Attach substitutes onto InventoryPart objects (for UI)
        attached = 0
        for base_item, subs in subs_by_base.items():
            inv = self._inv_by_itemnum.get(base_item)
            if not inv:
                continue
            
            # Normalize the base/preferred MPN from the daily inventory sheet
            base_vendor_key = DataLoader.norm_mpn_key(getattr(inv, "vendoritem", "") or "")

            added_any = False
            for s in subs:
                sub_key = DataLoader.norm_mpn_key(getattr(s, "mfgpn", "") or "")

                # Skip "self" (preferred MPN row showing up in the master export)
                if base_vendor_key and sub_key and sub_key == base_vendor_key:
                    continue
                
                inv.add_substitute(s)
                added_any = True

            if added_any:
                attached += 1

        self._alt_mpn_to_base = mpn_to_base
        self._alt_loaded = True
        return attached, len(mpn_to_base)
                    ...
                    if len(bases) == 1:
                        ...
                        if inv is not None:
                            match = MatchResult(
                                ...
                                inventory_part=inv,
                                candidates=[inv],
                                notes="Resolved via alternates DB",
                                # conflict attempt (don’t auto-pick)
                                explain = {
                                    "substitute_mpn": mpn,
                                    "bases": bases,
                                    "attempts": [
                        ...
                        # Conflict: same MFGPN appears under multiple base items.
                        # Don’t silently choose; fall back to ML matching.
                        pass
...
#
    ...
    def _pair_to_node(self, npr_part: Any, match: Any) -> DecisionNode:
        ...
        # --- propagate winner explicitly into node.explain ---
        node.explain = dict(safe_get(match, "explain", default={}) or {})
        node.explain["winning_mpn"] = winning_mpn

        # If the match came from alternates DB resolution, preserve the base itemnum too.
        # (Our MatchResult.explain for substitutes sets these.)
        base_itemnum = None
        ex = node.explain or {}
        if isinstance(ex, dict):
            base_itemnum = ex.get("base_itemnum") or ex.get("winning_base_itemnum")
        node.explain["winning_base_itemnum"] = base_itemnum

        # Winner marking: support both the old scheme (customer_mpn match) and substitute scheme (base match)
        for a in node.explain.get("attempts", []):
            a_customer = a.get("customer_mpn")
            a_base = a.get("base_itemnum") or a.get("resolved_base_itemnum")
            a["is_winner"] = (
                (winning_mpn is not None and a_customer == winning_mpn) or
                (base_itemnum is not None and a_base == base_itemnum)
            )
