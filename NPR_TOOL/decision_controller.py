...
#------------------------------------    
    ...
    def _pair_to_node(self, npr_part: Any, match: Any) -> DecisionNode:
        ...
        # Use BOM row ID as the *real* stable unique key when possible
        bom_row_id = None
        try:
            bom_row_id = (self._bom_row_by_uid or {}).get(bom_uid)
        except Exception:
            bom_row_id = None

        #node_id = bom_uid or (internal_pn or f"NODE-{len(self.nodes)+1}")
        node_id = bom_row_id or bom_uid or internal_pn or f"NODE-{len(self.nodes)+1}"
